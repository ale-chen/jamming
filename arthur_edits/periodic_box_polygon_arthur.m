classdef periodic_box_polygon_arthur < handle & matlab.mixin.Copyable
  properties
  ...
  methods
    ...
    function [pressure, e1,e2,e3] = iterate_time(obj, dt)
        ...
        old_force = zeros(size(obj.polygons,2),2);
        new_q = zeros(size(obj.polygons,2),2);
        new_theta = zeros(size(obj.polygons,2),2);

        old_torque = zeros([size(obj.polygons,2),1]);
        
        for i = 1:size(obj.polygons,2)

            newq = obj.polygons(i).q + obj.polygons(i).v * dt +...
                ((1/(2*obj.polygons(i).mass_tot)) * obj.polygons(i).force) * (dt^2); % APPLY PBC !!NOT DONE YET!! FIGURE OUT IF I WANT TO SCALE TO MASS (ask mark)
            
            if true
                dtheta = obj.polygons(i).w * dt +...
                    ((1/(2*(obj.polygons(i).mofi))) * obj.polygons(i).torque) * (dt^2);
                newtheta = obj.polygons(i).theta + dtheta;
            
                obj.polygons(i).theta = newtheta;
                obj.polygons(i).rotate_vertices(dtheta)
            end

            old_force(i,:) = obj.polygons(i).force;
            old_torque(i) = obj.polygons(i).torque;
            % disp('new_theta')
            % disp(newtheta)
        ...
        for i = 1:size(obj.polygons,2)
            ...
            [radial_force_t,torque_t, PE_contrib] = obj.total_force_torque(i);
            e3_other = e3_other + PE_contrib/2;
            % calculate energy goes here

            obj.polygons(i).force = radial_force_t;
            obj.polygons(i).torque = torque_t;

            if any(radial_force_t>0)
                'hi';
            end
        ...
        [e1,e2,~] = obj.get_energy();

        obj.t = obj.t + dt;
        pressure = sum(radial_force_magnitudes)*(1/(2*obj.get_area));
    end

    function [F_tot, torque, PE] = total_force_torque(obj, polygon_id)
        ...
        for vertex = 1:size(main_vertices, 1)
            ...
            force_radial = vertex_tot;%r_unit * force_radial_mag;
