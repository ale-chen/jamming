...
pressure = 101; 
while pressure > 100
    % Calculate spacing between polygons
    spacing = 1.5 * sigma;
    ...
    for i = 1:num_polygons
        ...
        q = [xmin + col * spacing, ymin + row * spacing];
        v = rand(1,2)*5 - 2.5;
        w = 0;
...
function [t_values, q_series, v_series, theta_series, w_series,...
    ...
    xmin_series = ones(1, num_steps) * box.xmin;
    xmax_series = ones(1, num_steps) * box.xmax; 
    ymin_series = ones(1, num_steps) * box.ymin;
    ymax_series = ones(1, num_steps) * box.ymax;
    ...
    for step = 1:num_steps
        ...
        if mod(step, 10) == 0
            ...
            for polygon = 1:num_polygons
                ...
                plot(vertices(:,1), vertices(:,2), 'o-', 'Color', colors(polygon,:), 'LineWidth', 1.5, 'MarkerSize', sigma);
            end
